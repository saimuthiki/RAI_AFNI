...
- **`garak-main`** — CORRECTION: shields.Up/Down DETECTORS exist but ship with NO matching probe module - the 'point shields up/down at AFNI's gateway' work item needs AFNI to write the probe. Attacker LLMs default to local HF; only judge.* detectors need a paid key.
...
- **`Guardrails-develop`** — v0.24.0.dev0. Jailbreak rail is documented FAIL-OPEN (jailbreak-protection.mdx:112) - confirms the flip-to-fail-closed decision. Heuristics pull gpt2-large. yara-python is an extra.
