...
title: "Dialog Rails"
description: "This section explains how to create dialog rails using Colang."
---

<a id="colang_2_getting_started_dialog_rails"></a>
...
The example below extends the [Hello World example](hello-world#colang_2_getting_started_hello_world) by creating the `user expressed greeting` and `bot express greeting` messages.
...
To get a better understand of what's happening under the hood, you can use the `--verbose` flag when launching the NeMo Guardrails CLI to show all the processed events:
...
The [next example](multimodal-rails#colang_2_getting_started_multimodal_rails) will show you how to describe multimodal rails using Colang.
