...
The `self check input` rail [prompts](/configure-guardrails/guardrail-catalog/self-check#self-check-input) an LLM model to check if the input is safe for the bot to process.
