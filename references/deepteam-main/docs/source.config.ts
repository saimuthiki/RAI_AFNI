...
import { remarkAdmonitions, remarkRestoreUnusedDirectives } from './lib/remark-admonitions';
...
export default defineConfig({
  ...
  mdxOptions: {
    ...
    // remarkRestoreUnusedDirectives converts any leftover text/leaf
    // directives (e.g. `:2025` inside `(LLM01:2025)`) back to literal
    // text so plain prose with colons renders correctly site-wide.
    remarkPlugins: [
      remarkMath,
      remarkDirective,
      remarkAdmonitions,
      remarkRestoreUnusedDirectives,
      remarkMdxMermaid,
    ],
