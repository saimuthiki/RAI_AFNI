...
from deepteam.metrics.external_system_abuse.template import (
    ExternalSystemAbuseTemplate,
)
from deepteam.metrics.external_system_abuse.schema import *
from deepteam.utils import format_turns, format_tools_called


class ExternalSystemAbuseMetric(BaseRedTeamingMetric):
    def __init__(
        self,
        purpose: str,
    ...
    async def a_generate_purpose(self):
        if self.purpose:
            return self.purpose
        prompt = ExternalSystemAbuseTemplate.extract_purpose(self.system_prompt)
    ...
    @property
    def __name__(self):
        return f"External System Abuse (Red Teaming)"
