...
from deepteam.metrics import ChildProtectionMetric, BaseRedTeamingMetric
...
class ChildProtection(BaseVulnerability):
    ...
    def __init__(
        ...
        types: Optional[List[ChildProtectionLiteralType]] = [
        ...
        enum_types = validate_vulnerability_types(
            self.get_name(), types=types, allowed_type=ChildProtectionType
        )
    ...
    def assess(
        ...
        purpose: Optional[str] = None,
    ) -> Dict[ChildProtectionType, List[RTTestCase]]:
        ...
        results: Dict[ChildProtectionType, List[RTTestCase]] = {}
        res: Dict[ChildProtectionType, ChildProtectionMetric] = {}
    ...
    def simulate_attacks(
        ...
        for type in self.types:
            ...
            templates[type].append(
                ChildProtectionTemplate.generate_baseline_attacks(
    ...
    def _assess_trace(
        ...
        """
        Evaluates an entire execution trace for Child Protection vulnerabilities using bottoms-up batching.
        """
        if self.async_mode:
            loop = get_or_create_event_loop()
        ...
        trace_scanner = TraceScanner(
            ...
            template=ChildProtectionTemplate,
    ...
    async def _a_assess_trace(
        ...
        """
        Asynchronously evaluates an entire execution trace for Child Protection vulnerabilities.
        """
        self.evaluation_model, self.using_native_model = initialize_model(
            self.evaluation_model
        )
    ...
    def _get_metric(
        self,
        type: ChildProtectionType,
    ) -> BaseRedTeamingMetric:
        return ChildProtectionMetric(
