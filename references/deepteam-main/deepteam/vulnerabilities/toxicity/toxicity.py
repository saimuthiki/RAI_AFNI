...
from deepteam.metrics import ToxicityMetric, BaseRedTeamingMetric
...
class Toxicity(BaseVulnerability):
    ...
    def assess(
        ...
        results: Dict[ToxicityType, List[RTTestCase]] = {}
        res: Dict[ToxicityType, ToxicityMetric] = {}
    ...
    def _assess_trace(
        ...
        """
        Evaluates an entire execution trace for Toxicity vulnerabilities using bottoms-up batching.
        """
        if self.async_mode:
            loop = get_or_create_event_loop()
    ...
    async def _a_assess_trace(
        ...
        """
        Asynchronously evaluates an entire execution trace for Toxicity vulnerabilities.
        """
        self.evaluation_model, self.using_native_model = initialize_model(
            self.evaluation_model
        )
