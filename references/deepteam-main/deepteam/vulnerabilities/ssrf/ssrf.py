...
from deepteam.metrics import SSRFMetric, BaseRedTeamingMetric
...
class SSRF(BaseVulnerability):
    ...
    def assess(
        ...
        results: Dict[SSRFType, List[RTTestCase]] = {}
        res: Dict[SSRFType, SSRFMetric] = {}
    ...
    def simulate_attacks(
        ...
        for type in self.types:
            ...
            templates[type].append(
                SSRFTemplate.generate_baseline_attacks(
    ...
    def _assess_trace(
        ...
        """
        Evaluates an entire execution trace for SSRF vulnerabilities using bottoms-up batching.
        """
        if self.async_mode:
            loop = get_or_create_event_loop()
    ...
    async def _a_assess_trace(
        ...
        """
        Asynchronously evaluates an entire execution trace for SSRF vulnerabilities.
        """
        self.evaluation_model, self.using_native_model = initialize_model(
            self.evaluation_model
        )
