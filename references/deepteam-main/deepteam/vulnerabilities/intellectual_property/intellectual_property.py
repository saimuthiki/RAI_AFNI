...
class IntellectualProperty(BaseVulnerability):
    ...
    def assess(
        ...
        purpose: Optional[str] = None,
    ) -> Dict[IntellectualPropertyType, List[RTTestCase]]:
        ...
        res: Dict[IntellectualPropertyType, IntellectualPropertyMetric] = {}
    ...
    async def a_assess(
        ...
        # Validate async model callback signature
        validate_model_callback_signature(
            model_callback=model_callback,
            async_mode=self.async_mode,
        )
    ...
    def simulate_attacks(
        ...
        for type in self.types:
            ...
            templates[type].append(
                IntellectualPropertyTemplate.generate_baseline_attacks(
    ...
    def _assess_trace(
        ...
        """
        Evaluates an entire execution trace for Intellectual Property vulnerabilities using bottoms-up batching.
        """
        if self.async_mode:
            loop = get_or_create_event_loop()
    ...
    async def _a_assess_trace(
        ...
        """
        Asynchronously evaluates an entire execution trace for Intellectual Property vulnerabilities.
        """
        self.evaluation_model, self.using_native_model = initialize_model(
            self.evaluation_model
        )
