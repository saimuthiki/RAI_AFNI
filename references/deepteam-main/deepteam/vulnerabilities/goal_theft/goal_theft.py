...
class GoalTheft(BaseVulnerability):
    ...
    ALLOWED_TYPES = [type.value for type in GoalTheftType]
    category = "Agentic"
    ...
    def _assess_trace(
        ...
        """
        Evaluates an entire execution trace for Goal Theft vulnerabilities using bottoms-up batching.
        """
        if self.async_mode:
            loop = get_or_create_event_loop()
    ...
    async def _a_assess_trace(
        ...
        """
        Asynchronously evaluates an entire execution trace for Goal Theft vulnerabilities.
        """
        self.evaluation_model, self.using_native_model = initialize_model(
            self.evaluation_model
        )
