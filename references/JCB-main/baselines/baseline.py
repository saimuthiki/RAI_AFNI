...
class SingleBehaviorRedTeamingMethod(RedTeamingMethod):
    ...
    def generate_test_cases(self, base_save_dir, behaviors, verbose=False):
        ...
        logs = {}
        jailbroken_list=[]
        main_batch_total_LLM_queries = 0
        random.shuffle(behaviors)
        print(len(behaviors))
        attack_success_completions = {}
        for behavior_dict in tqdm(behaviors, total=len(behaviors)):
            ...
            for j in range(num_batches):
                ...
                batch_test_cases, batch_logs, batch_total_LLM_queries, best_completion, jailbroken = self.generate_test_cases_single_behavior(behavior_dict, num_generate=current_batch_size,
                                                                                        verbose=verbose)
                main_batch_total_LLM_queries+=batch_total_LLM_queries
                print(f"overall total queries so far: {main_batch_total_LLM_queries}")
                attack_success_completions[behavior_dict['BehaviorID']] = [dict(test_case=batch_test_cases, generation=best_completion)]
                ...
                if main_batch_total_LLM_queries>=4000:
                    term=True
                    break
            ...
            if term:
                print(f"Reached {main_batch_total_LLM_queries} (which is >=4000) total queries so terminating")
                break
        ...
        save_path = os.path.join(base_save_dir, "PAIR", experiment_name, 'completions', f'{model_name}.json')
        ...
        print(f"Total jailbroken: {sum(jailbroken_list)}")
        print(f"GPT ASR: {sum(jailbroken_list)*100/len(behaviors)}%")
    ...
    def save_test_cases_single_behavior(self, save_dir, behavior_id, test_cases, logs=None, method_config=None, run_id=None):
        if save_dir is None:
            return
        # print("In save_test_cases_single_behavior")
