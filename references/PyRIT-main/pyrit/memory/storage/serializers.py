...
async def set_message_piece_sha256_async(message_piece: MessagePiece) -> None:
    """
    Compute and assign SHA256 hash values for a message piece's original and converted payloads.

    Async because blob payloads may need to be fetched. Must be called explicitly after
    the message piece is constructed and its values are finalized.

    Args:
        message_piece (MessagePiece): The message piece to populate with SHA256 values.
    """
    original_serializer = data_serializer_factory(
        category="prompt-memory-entries",
        data_type=message_piece.original_value_data_type,
        value=message_piece.original_value,
    )
    message_piece.original_value_sha256 = await original_serializer.get_sha256_async()
    ...
    message_piece.converted_value_sha256 = await converted_serializer.get_sha256_async()


async def set_seed_sha256_async(seed: Seed) -> None:
    """
    Compute and assign the SHA256 hash value for a seed's value.

    Should be called after the seed ``value`` is serialized to text, as file paths used in
    the ``value`` may have changed from local to memory storage paths. Async due to blob retrieval.

    Args:
        seed (Seed): The seed to populate with its SHA256 value.
    """
    ...
    seed.value_sha256 = await serializer.get_sha256_async()
